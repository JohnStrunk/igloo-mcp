...
async def main():
    ...
    async with streamable_http_client("http://localhost:8000/mcp") as (
        ...
        async with ClientSession(read_stream, write_stream) as session:
            ...
            print(f"{'=' * 60}")
            print(
                f"Calling the 'search_content' tool with arguments: {search_tool_args}"
            )
            ...
            member_result = await session.call_tool(
                "search_members", search_member_args
            )
            ...
            if member_result.isError:
                ...
                if member_id_match:
                    ...
                    profile_result = await session.call_tool(
                        "fetch_members", fetch_member_args
                    )
